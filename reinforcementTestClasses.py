...
import time
from util import Counter, FixedRandom, Experiences
...
def get_question_test_from_filepath(path):
    if sys.platform == 'win32':
        question_name, test_name = path.split('\\')[-2:]
    else:
        # SS: fixed it to allow autograder from any path
        #_, question_name, test_name = testDict['test_out_file'].split('/')
        question_name, test_name = os.path.normpath(path).split(os.sep)[-2:]
    return question_name, test_name
...
class ApproximateQLearningTest(testClasses.TestCase):
    ...
    def __init__(self, question, testDict):
        ...
        self.testOutFile = testDict['test_out_file']
        question_name, test_name = get_question_test_from_filepath(testDict['test_out_file'])
...
class EvalAgentTest(testClasses.TestCase):
    ...
    def __init__(self, question, testDict):
        ...
        # SS: added to pass the student code to pacman.py
        if "student_code" in testDict:
            self.pacmanParams += f" --student-code {testDict['student_code']}"
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        startTime = time.time()
        args_run_games = pacman.readCommand(self.pacmanParams.split(' '))
        games = pacman.runGames(**args_run_games)
